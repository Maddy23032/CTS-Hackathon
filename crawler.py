...
class Crawler:
    ...
    def _get_fresh_tokens(self, form_url):
        """Get fresh CSRF tokens for a form URL (like Burp Suite auto-tracking)"""
        # Look for tokens in the same page or parent pages
        for url, tokens in self.csrf_tokens.items():
            if url == form_url or form_url.startswith(url.rstrip('/')):
                return tokens
        return {}

    def _update_session_state(self, response):
        """Track session state changes (cookies, headers, etc.)"""
        # Update session cookies
        if response.cookies:
            for cookie in response.cookies:
                self.session_state[cookie.name] = cookie.value
                self.log(f"[Session] Cookie updated: {cookie.name} = {cookie.value[:20]}...")
        
        # Track other session indicators
        if 'set-cookie' in response.headers:
            self.log(f"[Session] Session state updated from response headers")

    def _simulate_user_interactions(self, page):
        """Simulate user interactions to discover dynamic content (like Burp Suite)."""
        try:
            self.log("[Browser] Simulating user interactions to discover dynamic content...")
            
            # Scroll to trigger lazy-loading content
            page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
            page.wait_for_timeout(500)
            
            # Click on clickable elements that might reveal content
            clickable_selectors = [
                'button:not([type="submit"])',  # Non-submit buttons
                '[role="button"]',              # ARIA buttons
                '.tab, .nav-tab',              # Tab navigation
                '.dropdown, .menu-toggle',      # Dropdowns
                '[onclick]',                   # Elements with onclick handlers
                'a[href="#"]',                 # Hash links (often JS-powered)
            ]
            
            for selector in clickable_selectors:
                elements = page.query_selector_all(selector)
                for i, element in enumerate(elements[:3]):  # Limit to 3 per type to avoid infinite loops
                    try:
                        if element.is_visible():
                            self.log(f"[Browser] Clicking {selector} element {i+1}")
                            element.click(timeout=1000)
                            page.wait_for_timeout(500)  # Wait for potential dynamic content
                    except Exception as e:
                        self.log(f"[Browser] Click failed on {selector}: {e}")
                        continue
            
            # Hover over elements that might trigger dynamic content
            hover_selectors = ['.menu', '.nav', '[data-hover]']
            for selector in hover_selectors:
                elements = page.query_selector_all(selector)
                for element in elements[:2]:  # Limit hover actions
                    try:
                        if element.is_visible():
            ...
            # Trigger form focus events that might reveal hidden fields
            form_inputs = page.query_selector_all('input, select, textarea')
            for inp in form_inputs[:5]:  # Limit to prevent excessive interactions
                try:
                    if inp.is_visible():
                        inp.focus()
            ...
            self.log(f"[Browser] User interaction simulation failed: {e}")
        ...
        if self.use_browser:
            try:
                ...
                with sync_playwright() as p:
                    ...
                    # Simulate user interactions to discover dynamic content
                    ...
                    # Discover all links (including dynamically generated ones)
                    links = []
                    for a in page.query_selector_all('a[href], [onclick], button[data-href], [data-url]'):
                        href = a.get_attribute('href') or a.get_attribute('data-href') or a.get_attribute('data-url')
                        if href:
                            links.append(urljoin(url, href))
                    
                    # Discover forms (including dynamic ones)
                    forms = []
            ...
            try:
                ...
                self._update_session_state(resp)
